import { assetGender, lockGenderProfile } from "../../shared/flow/gender.mjs";
export const FLOW_VOICE_PROFILES = [
  {
    character: "Doctor",
    baseVoice: "Alnilam",
    type: "Male",
    ...
    aliases: ["doctor", "marcus", "marcus-meat-first", "cardiologist", "surgeon", "heart", "avatar profile 2"],
  },
  {
  ...
  },
  {
    ...
    aliases: ["parisian woman", "parisian", "colette", "colette-de-paris", "paris", "french", "avatar profile 4"],
  },
  {
    ...
    aliases: ["latino dad", "latino", "papa-diego", "diego", "papa", "hispanic", "avatar profile 5"],
  },
  {
    character: "Trucker",
    baseVoice: "Schedar",
  ...
  },
  {
    ...
    aliases: ["street doctor", "coach-andre", "andre", "coach", "street", "atlanta", "avatar profile 8"],
  },
  {
    character: "Ohio Mom",
    baseVoice: "Achernar",
  ...
  },
  {
    ...
    aliases: ["hong kong woman", "hong kong", "mei-quiet-glow", "mei", "hongkong", "chinese", "avatar profile 10"],
  },
  {
    ...
    aliases: ["african elder", "elder-emeka", "emeka", "elder", "african", "avatar profile 1"],
  },
];
...
function resolveUnrestrictedFlowVoice({ name, handle, voiceName } = {}) {
  const wanted = normalize(voiceName);
  if (wanted) {
    const exact = FLOW_VOICE_PROFILES.find(
      (p) => normalize(p.character) === wanted || normalize(p.voiceName) === wanted,
    );
    if (exact) return exact;
  }
  const hay = [handle, name, voiceName].filter(Boolean).map((s) => normalize(s));
  ...
  return FLOW_VOICE_PROFILES[0];
}

/** Flow stock voices: pitch 1 = lower, 5 = high. */
export const FLOW_PRESET_TRAITS = {
...
/** Flow overlay chrome — never treat these as a character or voice. */
export const FLOW_ASSET_CHROME_RE =
  /navigation|category|asset list|sort assets|search assets|add to prompt|upload media|tile grid|filtering|accessibility|project navigation|select project|more options|more_vert/i;

export function isFlowAssetChrome(name) {
  const n = String(name || "").trim();
  if (!n) return true;
  if (FLOW_ASSET_CHROME_RE.test(n)) return true;
  return /^(voices|all|images|videos|characters|uploads|recent|tab|filter|sort|search|close|done|cancel|menu|settings|home|back)$/i.test(
    n,
  );
}

export function realAssetNames(available = []) {
  return [...new Set((available || []).map((s) => String(s || "").trim()).filter((n) => n && !isFlowAssetChrome(n)))];
}

export function rankAvailableVoices(profile, available = []) {
  const wantedPitch = FLOW_PRESET_TRAITS[profile?.baseVoice]?.pitch ?? (profile?.type === "Female" ? 3 : 2);
  const custom = normalize(profile?.voiceName);
  const character = normalize(profile?.character);
  const base = normalize(profile?.baseVoice);
  const names = realAssetNames(available).filter(name => !profile?.genderLock || assetGender(name, FLOW_VOICE_PROFILES, FLOW_PRESET_TRAITS) === profile.genderLock);
...
export function rankAvailableCharacters(profile, available = []) {
  const wanted = normalize(profile?.character);
  const voice = normalize(profile?.voiceName);
  const aliases = [profile?.character, profile?.voiceName, ...(profile?.aliases || [])]
    .map((s) => normalize(s))
    .filter(Boolean);
  const names = realAssetNames(available).filter(name => !profile?.genderLock || assetGender(name, FLOW_VOICE_PROFILES) === profile.genderLock);
...
export function resolveFlowVoice(input = {}) {
  return lockGenderProfile(resolveUnrestrictedFlowVoice(input), input.characterGender, FLOW_VOICE_PROFILES);
}
