...
import {clipLiveLabel,projectProgress,type ProjectStatus} from '@/lib/experimental/project-progress';
...
export function ProjectClips({reelId,processing}:{reelId:number;processing:boolean}){
 ...
 return <section aria-label="Project clip downloads" className="space-y-5">
  ...
  <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{clips.map(clip=><article key={clip.id} className="min-w-0 overflow-hidden rounded-lg border">
   {clip.available?<video aria-label={`Preview ${clip.id}`} src={`/api/experimental/reels/${reelId}/clips/${clip.id}/video`} controls playsInline preload="none" className="h-72 w-full bg-muted object-contain"/>:<div className="flex h-48 items-center justify-center bg-muted p-6 text-center text-sm text-muted-foreground">{busy?<span className="flex flex-col items-center gap-3"><LoaderCircle aria-hidden className="size-6 animate-spin motion-reduce:animate-none"/>{clipLiveLabel(live,clip.id)??(progress.clipId===clip.id?progress.message:'Waiting for this clip')}</span>:clip.status==='missing'?'Saved file is unavailable':'This clip has not been completed'}</div>}
