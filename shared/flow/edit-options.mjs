import { normalizeSubtitleStyle } from './subtitle-style.mjs';
...
export function normalizeEditOptions(value) {
  ...
  const allowed = ['transition', 'subtitles', 'subtitleSize', 'subtitlePosition', 'subtitleFade', 'subtitleStyle'];
  ...
  return { ...(value.transition ? {transition:value.transition} : {}), subtitles:value.subtitles ?? true, subtitleSize:value.subtitleSize ?? 54, subtitlePosition:value.subtitlePosition ?? 'bottom', subtitleFade:value.subtitleFade ?? true, ...(value.subtitleStyle !== undefined ? {subtitleStyle:normalizeSubtitleStyle(value.subtitleStyle)} : {}) };
}
